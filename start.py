        ...
        try:
            ...
            if self.is_windows:
                ...
                process = subprocess.Popen([
                    "start", "cmd", "/k", f"cd /d {django_dir} && {python_path} manage.py runserver 0.0.0.0:8000"
                ], shell=True)
            else:
                # Linux/Mac下启动后台进程
                process = subprocess.Popen([
    ...
    def start_face_recognition(self):
        ...
        try:
            ...
            if self.is_windows:
                # Windows下启动新窗口
                process = subprocess.Popen([
                    "start", "cmd", "/k", f"cd /d {face_dir} && {python_path} face_api.py"
                ], shell=True)
            else:
                # Linux/Mac下启动后台进程
                process = subprocess.Popen([
    ...
    def start_frontend(self):
        ...
        try:
            ...
            if self.is_windows:
                # Windows下启动新窗口
                process = subprocess.Popen([
                    "start", "cmd", "/k", f"cd /d {self.base_dir} && {npm_cmd} run dev"
                ], shell=True)
            else:
                # Linux/Mac下启动后台进程
                process = subprocess.Popen([
